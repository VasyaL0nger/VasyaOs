...
# База данных о Василии для ИИ (Обучающий контекст)
VASYA_BIO = (
...
# Сверхстабильная функция общения с ИИ без ключей и блокировок
def ask_unlimited_ai(system_prompt, user_question):
    try:
        # Используем открытый и бесплатный прокси-сервер Pollinations AI с моделью openai-large
        # Он работает без токенов, мгновенно и понимает системные роли
        messages = [
        ...
        if response.status_code == 200 and response.text:
            ...
            return "Мяу... Мой кошачий процессор на секунду задумался. Нажми кнопку еще раз, я обязательно отвечу!"
    except:
        return "Василий ушел греться к костру. Пожалуйста, повтори отправку сообщения!"
...
if model_choice == "VasyaTheCat (Болталка)":
    ...
    system_prompt = f"Ты — сам кот Василий. Твоя биография: {VASYA_BIO}. Отвечай лениво, гордо, по-королевски, как харизматичный кот. Используй кошачьи фразочки, иногда вставляй 'мяу' и требуй пельмени за общение. Ты можешь рассуждать на любые темы, но всегда помни, что ты кот. Отвечай СТРОГО на русском языке."
    ...
    system_prompt = f"Ты — эксперт по коту Василию. Опирайся на реальные факты из базы данных: {VASYA_BIO}. Твоя задача — давать развернутые, умные и четкие ответы на любые вопросы пользователей, связывая их с жизнью Василия. Отвечай СТРОГО на русском языке."
    ...
    system_prompt = f"Ты — вежливый ИИ-гид 'VasyaAI'. Рассказывай красиво, вежливо и развернуто про кота Василия на основе фактов: {VASYA_BIO}. Если пользователь спрашивает отвлеченные вещи, вежливо связывай их с историей кота. Отвечай СТРОГО на русском языке."
    ...
    system_prompt = f"Ты — талантливый поэт-юморист. Твоя цель — сочинять забавные, новые, складные стихотворения с четкой рифмой про кота Василия, используя его привычки (пельмени, костер, кресло): {VASYA_BIO}. Пиши стихи на любую тему, которую предложит пользователь, но делай Василия главным героем. Отвечай СТРОГО на русском языке."
...
if model_choice != "CanvasVasya (Арт)":
    ...
    if st.button("Отправить запрос"):
        if user_input:
            with st.spinner("VasyaIQ обрабатывает данные..."):
                response = ask_unlimited_ai(system_prompt, user_input)
                st.write("---")
